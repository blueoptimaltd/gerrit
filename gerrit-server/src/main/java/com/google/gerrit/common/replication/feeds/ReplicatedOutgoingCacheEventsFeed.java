...
public class ReplicatedOutgoingCacheEventsFeed extends ReplicatedOutgoingEventsFeedCommon {
  ...
  /**
   * replicateEvictionFromCache is used to evict specific cache entries from remote servers.
   * The servers that are communicated with are specified by all-users / all-projects if you supply no project.
   * Caches which should be on specific projects, should call the overridden method giving the relative project name.
   * @param cacheName
   * @param key
   */
  public void replicateEvictionFromCache(final String cacheName, final Object key) {
    replicateEvictionFromCache(cacheName, key, null);
  }

  /**
   * replicateEvictionFromCache is used to evict specific cache entries from remote servers.
   * The servers that are communicated with are specified by the project used.
   * Note: Caches in the all-users eviction list will still be sent to all-users sites regardless.
   * @param cacheName
   * @param key
   * @param suppliedProjectName
   */
  public void replicateEvictionFromCache(final String cacheName, final Object key, final String suppliedProjectName ) {
    ...
    try {
      ...
      if (getAllUsersCacheEvictList().contains(cacheName)) {
        // Block to force cache update to the All-Users repo, this is forcing an update to the project name, usually because we didn't
        // understand the cache key name class correctly.
        projectName = replicatedEventsCoordinator.getReplicatedConfiguration().getAllUsersName();
      } else if (Strings.isNullOrEmpty(suppliedProjectName)) {
        // no project was supplied, lets default this to All-Projects ( as its not an All-Users cache ).
        projectName = replicatedEventsCoordinator.getReplicatedConfiguration().getAllProjectsName();
      } else {
        //Set eventWrapper to a project specific EventWrapper, if no project is specified, it will send to ALL-Projects.
        projectName = suppliedProjectName;
      }

      log.debug("CACHE About to call replicated cache eviction for  Project: [ {} ], CacheName: [ {} ], Key: [ {} ]", projectName, cacheName, key);
  ...
  public void replicateMethodCallFromCache(final String cacheName,
                                           final String suppliedProjectName,
    ...
    try {
      replicatedEventsCoordinator.queueEventForReplication(
          GerritEventFactory.createReplicatedCacheEvent(suppliedProjectName, cacheMehodCall));
    } catch (IOException e) {
